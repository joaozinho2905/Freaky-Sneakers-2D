...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['acao']) && $_POST['acao'] === 'excluir' && isset($_POST['id_usuario'])) {
    $id_usuario = $_POST['id_usuario'];
    $sql_delete = "DELETE FROM voluntarios WHERE id_usuario = ?";
    $stmt = $conexao->prepare($sql_delete);
    $stmt->bind_param("i", $id_usuario);
    if ($stmt->execute()) {
        $stmt->close();
        header("Location: ../Pag5-Busca/buscavoluntarios.php?msg=excluido");
        exit();
    } else {
        error_log("Erro ao excluir voluntário: " . $stmt->error);
    }
    $stmt->close();
}
